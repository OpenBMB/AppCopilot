...
如图是 “搜索栏筛选” 路径，展现了智能体在**主动控制**需求边界场景下的策略。这一路径的核心逻辑是**精准拆解需求、分步实现目标**，首先在搜索栏输入 “餐厅” 触发基础搜索，确保覆盖全量目标对象；接着在结果页找到 “距离” 筛选选项，手动选择 “附近”，将空间范围锁定；最后在排序菜单中切换至 “好评优先”，按评分高低重新组织结果。这种分步操作的每一步都直接对应指令中的核心条件，通过手动设置筛选参数确保结果的精准性。

本任务顺利完成了在大众点评查看附近评分最高的餐厅，反映了智能体在复杂应用场景中多策略适配的核心能力。
...
如图，本任务中丽丽的手机存储 5G 宽视界的历史观看记录数据，用户的手机基于观看记录数据完成礼物购买，展现出的**跨设备多智能体协作、用户偏好提取、跨应用决策及隐私合规处理**能力，不仅实现了从行为数据到**个性化服务**的闭环，更凸显了智能化交互在跨端场景中的技术突破。

指令执行初期，在通过权限验证机制确认已获得数据访问授权的基础上，Agent 精准定位 5G 宽视界的历史记录模块，提取最近一条视频的关键信息，通过模型完成内容解析与关键词提取，确保数据以结构化形式传递。但原始视频列表无法直接指导礼物选择，在任务中，Agent 从丽丽观看的 “蜡笔小新” 视频中提取出 IP 关键词，进而推断出其潜在兴趣点，为后续礼物选择提供了明确方向。这一过程超越了简单的数据搬运，而是通过内容理解实现了数据到偏好到需求的跃升，体现了智能体对用户行为背后潜在诉求的洞察能力，让礼物推荐从盲目猜测变为精准匹配。

在用户的手机端，Agent 接收来自丽丽设备的 “蜡笔小新” 关键词后，自动启动淘宝应用，通过搜索功能定位相关礼物，Agent 在多步操作中能够保持流程的连贯性，在接收数据后无缝切换至淘宝，输入关键词并触发搜索，最终呈现符合偏好的购买选项。

尤为关键的是，该任务凸显了跨端服务的核心价值，打破设备壁垒，实现数据在端侧、服务在用户侧的精准对接。传统场景中，用户若想基于他人行为选礼，需手动询问偏好、自行搜索商品，流程繁琐且精准度低；而智能体通过跨端数据协同，自动完成数据采集到商品推荐的全流程，让服务跨越设备边界，直接落地于用户的操作场景中。

### Case 3: 三端任务

![Triple end Demo](../images/triple_end_cn.png)

如图，本任务中丽丽和凡凡的手机存储 5G 宽视界的历史观看记录数据，用户的手机基于观看记录数据完成礼物购买，展现了将**智能体能力从服务单个用户扩展到支撑多个用户的协同作业**。这一过程不是简单的技术叠加，而是从个人智能助理向分布式协作网络的范式转变，每位用户终端上运行的智能体代表着个体偏好与意图，通过自主发现、通信与协作，共同完成超出单体能力范围的复杂目标。

指令执行初期，丽丽和凡凡的智能体专注于解析自身 5G 宽视界历史，从观看记录中**提取个体偏好**，用户的智能体则作为协同方，接收并整合两端的偏好数据，驱动淘宝完成针对性选礼。这种架构下，每个智能体都拥有独立的计算空间与决策边界，既保留了代表个体意图的核心属性，又通过协同突破了单体智能的局限，即没有任何一个智能体能单独完成跨两人数据采集并且精准选礼的任务，但三者联动却能高效达成目标，这正是群体智能 “1+1>2” 的价值所在。

多端选礼任务印证了**分布式智能体系**的可行性。首先，针对不完整信息下的推理与协调挑战，丽丽与凡凡的智能体仅掌握自身终端的任务状态，对彼此的偏好与用户的选礼逻辑并无完整认知；用户的智能体也无法直接访问对方终端的原始数据，仅能基于加密传递的关键词进行推理。但通过**数据脱敏共享和意图识别机制**，智能体在信息不完整的情况下仍能精准协作，即丽丽的智能体无需知道凡凡的偏好，只需确保自身输出的关键词准确；用户的智能体无需了解视频的具体内容，仅通过关键词即可完成关联选礼。这种基于部分观测的协同能力，让智能体在保护数据隐私的同时，突破了终端与上下文的物理隔离。
其次，面对通信与协商机制构建挑战，智能体通过结构化协议和语义对齐实现交互。不同终端的智能体可能运行在异构系统中，但通过统一的端侧通信协议，实现了意图的精准传递。

多端选礼任务的顺利执行，验证**GUI智能体体系已从单智能体升级为具备多体协同、分布式状态建模与机制设计能力的系统级智能体架构**。这种升级的核心价值在于让智能服务突破了一人一机的边界，能够像人类社会的协作一样，**通过多个自主智能体的联动，完成复杂、跨域的长程任务**。当用户需要为家庭、团队甚至社群提供服务时，这种架构能自然扩展，新增用户的智能体只需遵循相同协议加入网络，即可无缝融入协同流程，真正迈向 “理论上可无限扩展至海量终端” 的群体智能愿景落地。
